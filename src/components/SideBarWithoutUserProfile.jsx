...
export default function MiniDrawer({ userName, open, setOpen, userRole/* , setUsername, setUserRole */ }) {
    ...
    <Box sx={{ display: 'flex' }}>
      ...
      <AppBar position="fixed" style={{backgroundColor: "#22168d"}}>
        <Toolbar>
          ...
          {userName && <div style={{display: 'flex', justifyContent: "flex-end", marginLeft: "auto" }}>
            <Typography variant="h6" style={{marginRight: 12, marginTop: 1}}>{userName.toUpperCase()}</Typography>
            <Button color="inherit" onClick={() => { localStorage.removeItem('token'), window.location = "/signin" }}>LogOut</Button>
          </div>
          }

          {!userName && <div style={{ marginLeft: 'auto' }}>
      ...
      <Box component="main" sx={{ flexGrow: 1, p: 3 }}>
        ...
        <Routes>
          ...
          <Route path="/" element={<Courses userRole={userRole} />} />
          <Route path="/signup" element={<Signup />} />
          <Route path="/signin" element={<Signin /* setUsername={setUsername} setUserRole={setUserRole} *//>} />
